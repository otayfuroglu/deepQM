...
PYTHON_DIR="$HOME/miniconda3/envs/deepqm/bin"
...
# model_list="aimnet2"
# model_list="nequip"
local_model_path="path/to/nequip"
...
$PYTHON_DIR/python $deepQM_DIR/deepQM.py $calcMode $n_procs $model_list $local_model_path $struct_dir $namebase $seq_start $seq_end $index_file_path $group1 $group2 $thr_fmax $maxiter

# For ANI_LIE: alpha=0.000, beta=0.127 and gamma=-5.111. For ANID3_LIE: alpha=-0.00353, beta=0.1487 and gamma=-5.9866.
# IMPORTANT: Solvent terms of LSinLS and LSinPLS from Summary.dat of those calculations must be explicitly given (e.g. -ls_in_ls -7.5 -ls_in_pls -4.0)
# $PYTHON_DIR/python $deepQM_DIR/scripts/bindEnAniD3.py -in "$struct_dir"/"$namebase"_SP_energies_"$group1"_"$group2".csv -ani ani2x -a -0.0353 -b 0.1487 -g -5.9866 -ls_in_ls -7.5 -ls_in_pls -4.0
